...
source("../algorithms/PurelyRF_fullgrown.R")
...
trees <- plyr::raply(
  100,
  random_reg_tree(formula = eq, data = data, minsize = 1),
  .progress = "text",
)
# extract fit
...
#print(ntable)
write.table(ntable, "Performance_Training_Set_nodesize_1", quote = FALSE, row.names = FALSE, col.names = TRUE)
...
write.table(test_table, "Performance_Test_Set_nodesize_1", quote = FALSE, row.names = FALSE, col.names = TRUE)
